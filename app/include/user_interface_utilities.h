...
{
    GraphicsController(SDL_Texture* sdl_texture_pointer)
...
static void set_emulation_screen_blank(GraphicsController& graphics_controller)
{
    ...
    if (ImGui::BeginMainMenuBar())
    {
        if (ImGui::BeginMenu("File"))
        {
                    ...
                    emulation_controller.game_boy_emulator.is_game_rom_loaded_in_memory_thread_safe()))
            {
                ...
                emulation_controller.game_boy_emulator.unload_game_rom_from_memory_thread_safe();
                emulation_controller.game_boy_emulator.reset_state();
            ...
            if (ImGui::MenuItem(
                    ...
                    "",
                    false,
                    emulation_controller.game_boy_emulator.is_boot_rom_loaded_in_memory_thread_safe()))
            {
                emulation_controller.is_emulation_paused_atomic.store(true, std::memory_order_release);
                emulation_controller.game_boy_emulator.unload_boot_rom_from_memory_thread_safe();

                if (emulation_controller.game_boy_emulator.is_boot_rom_mapped_in_memory() &&
                    emulation_controller.game_boy_emulator.is_game_rom_loaded_in_memory_thread_safe())
                {
                    emulation_controller.game_boy_emulator.reset_state();
                }
                    ...
                    IM_ARRAYSIZE(COLOUR_PALETTE_LABELS)))
            {
                switch (menu_properties.selected_colour_palette_combobox_index)
                {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        graphics_controller.active_colour_palette = custom_colour_palette;
                        break;
                }
    ...
    GraphicsController& graphics_controller)
{
    ...
    if (ImGui::BeginPopupModal("Custom Palette", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        for (int i = 0; i < 4; i++)
        {
            ...
            if (ImGui::ColorEdit4(
                    colour_label.c_str(),
                    reinterpret_cast<float *>(&selected_custom_colour_palette_colours[i]),
...
// Used in workaround for https://github.com/ocornut/imgui/issues/8339
static void sdl_logical_presentation_imgui_workaround_post_frame(
